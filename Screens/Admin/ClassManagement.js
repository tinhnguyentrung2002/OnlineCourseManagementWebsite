...
const ClassManagement = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const coursesQuery = query(coursesRef, where("course_type", "==", "class"), where("course_state", "==", true));
    ...
    try {
      ...
      if (courseSnapshot.exists()) {
        ...
        if (owner) {
          ...
          setEmailContent(`Chào bạn, lớp học ${courseTitle} với ID: ${courseId} hiện ${courseLock ? 'đã được mở khóa' : 'đã kết thúc do vi phạm tiêu chuẩn của hệ thống, vui lòng tạo một lớp học khác và điều chỉnh thông tin lớp học phù hợp'}.`);
  ...
  const handleViewCourse = (courseId) => {
    navigate(`/admin/lop-hoc/${courseId}`);
  };
  ...
  const columns = [
    { field: "course_id", headerName: "Mã lớp học", width: 200 },
    {
      field: 'course_img',
      headerName: 'Ảnh lớp học',
      width: 123,
      renderCell: (params) => (
        <img
    ...
    { field: "course_owner_id", headerName: "Giáo viên", width: 150, renderCell: (params) => getTeacherName(params.row.course_owner_id) },
    {
      ...
      renderCell: (params) => (
        <Badge bg={params.row.course_lock ? "danger" : "success"}>
          {params.row.course_lock ? "Đã kết thúc" : "Đang hoạt động"}
        </Badge>
      ),
    },
    {
      ...
      renderCell: (params) => (
        <div>
          <Button
            ...
            startIcon={params.row.course_lock ? <LockOpenIcon /> : <i class="fa-solid fa-stop"></i>}
          >
            {params.row.course_lock ? "Mở khóa" : "Kết thúc ngay"}
          </Button>
          <Button
            variant="contained"
            color="primary"
  ...
  const sendEmail = async () => {
    ...
    const templateParams = {
      ...
      subject: `Thông báo ${emailContent.includes('mở') ? 'mở khóa lớp học' : 'kết thúc lớp học'} ${courses.find(course => course.id === courseIdToSend)?.course_title}`,
