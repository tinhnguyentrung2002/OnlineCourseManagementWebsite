...
function Dashboard() {
  ...
  const fetchData = async () => {
    const avgRateQuery = query(collection(db, "Courses"), where("course_owner_id", "==", uid), where("course_type", "==", "course"));
    ...
    const totalStudentQuery = query(collection(db, "Courses"), where("course_owner_id", "==", uid));
    getDocs(totalStudentQuery).then(snapshot => {
      let totalMembers = 0;
      ...
      snapshot.forEach(doc => {
        ...
        if (data.course_type === "course") {
          totalMembers += data.course_member || 0;
        } else if (data.course_type === "class") {
          const classMembers = data.course_member || []; 
          totalMembers += classMembers.length; 
        }
      });

      setTotalStudent(totalMembers);
  }).catch(error => console.error("Lỗi khi thống kê số học viên: ", error));

    const totalCourseQuery = query(collection(db, "Courses"), where("course_owner_id", "==", uid));
    getCountFromServer(totalCourseQuery).then(countSnapshot => {
      setTotalCourse(countSnapshot.data().count);
    }).catch(error => console.error("Lỗi khi thống kê: ", error));

    const revenueQuery = query(collection(db, "Courses"), where("course_owner_id", "==", uid), where("course_type", "==", "course"));
    getDocs(revenueQuery).then(querySnapshot => {
      ...
      querySnapshot.forEach(doc => {
        ...
        totalRevenue += coursePrice * courseMembers * 0.8;
      });
  ...
  const fetchCourses = async () => {
    try {
      ...
      const coursesQuery = query(coursesCollection, where('course_owner_id', '==', uid));
  ...
  const fetchPricePieChartData = async () => {
    ...
    const q = query(coursesRef, where('course_owner_id', '==', uid));
  ...
  const fetchTypePieChartData = async () => {
    const coursesQuery = query(collection(db, 'Courses'), where('course_owner_id', '==', uid));
    const coursesSnapshot = await getDocs(coursesQuery);
  ...
  const exportToPDF = () => {
    ...
    const fileName = `ThongKe_GiaoVien_(${formattedDate})(${formattedTime}).pdf`;
  
    setTimeout(async () => {
      doc.setFontSize(20);
      doc.setFont('TimesNewRoman', 'bold');
      doc.text('Statistical Report - Role: Teacher', 60, 30);
      doc.setFontSize(10);
      ...
      doc.text(`Teacher: ` + userName , 90, 50);}else{  doc.text(`Teacher: Unknown ` , 90, 50);}
  ...
  const columns = [
    ...
    { field: "request_content", headerName: "Nội dung yêu cầu", width: 500 },
  ];
   ...
   <Box sx={{ display: 'flex', justifyContent: 'flex-start', marginBottom: '20px' }}>
            ...
            <Card className="card-stats">
              <Card.Body>
                <Row className="align-items-center" style={{ height: 123 }}>
                  ...
                  <Col xs="7">
                    <div className="numbers">
                      <p className="card-category">Học viên</p>
                ...
                <Row className="align-items-center" style={{ height: 123 }}>
                  ...
                  <Col xs="7">
                    <div className="numbers">
                      ...
                      <Card.Title as="h4">{avgRate}/5</Card.Title>
                    </div>
                  </Col>
                </Row>
              </Card.Body>
        ...
        <Row>
          <Col md="12">
            <Card>
              <Card.Header>
                <h4 className="card-title">Yêu cầu</h4>
              </Card.Header>
              <Card.Body>
              <Box>
              {loading ? (
